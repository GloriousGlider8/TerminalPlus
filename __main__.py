        ...
        if args[0] == "exit":
            ...
            if os.path.exists(f"{os.getenv("APPDATA")}\\TerminalPlus\\{ENVI}\\DEV"):
                if args[1] == "commit":
                    ...
                    os.system("git track addon-test.py")
                    if os.system(f"git commit -a -m \"Generated by T+ DEV: {input("[string] Commit message: ")}\"") == 0:
                        if input("[true / false] force changes: ") == "true":
                            if os.system("git push origin opendev --force") == 0:
                                print(f"{c.Fore.GREEN}Successfully opened a pull request to opendev branch!{c.Style.RESET_ALL}")
                            else:
                                print(f"{c.Fore.RED}Failed to push changes!{c.Style.RESET_ALL}")
                        else:
                            if os.system("git push origin opendev") == 0:
                                print(f"{c.Fore.GREEN}Successfully opened a pull request to opendev branch!{c.Style.RESET_ALL}")
                            else:
                                print(f"{c.Fore.RED}Failed to push changes!{c.Style.RESET_ALL}")
                        os.system("git commit -a -m \"Generated by T+ DEV: AUTO COMMIT\"")
                    else:
                        print(f"{c.Fore.RED}Failed to push changes!{c.Style.RESET_ALL}")
