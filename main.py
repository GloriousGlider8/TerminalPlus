...
if len(sys.argv) > 1:
    if sys.argv[1] == "/c" or sys.argv[1] == "-C":
        ...
        print(f"\n{c.Fore.BLUE}Terminal + Version {d.ver} Help for installation {d.env}{c.Style.RESET_ALL}")
        print("    /c -C <command> Execute the specified command (or commands separated with &&)")
        print("    /? --help       Display the help message shown now")
        print("    /ih --ihelp     Show the interactive help (ihelp) entry for Terminal +")
        print("    /u -U           Update to the latest version")
        print("    /s -S <version> Change to the version specified (must be cached) (not all versions support the same CLI arguments)")
        print()
    ...
    try:
        ...
        if args[0] == "exit":
            ...
            if f.selPrompt(["Yes", "No"], [">", "<"], "You may not have python installed correctly on your computer.\nUsing addons and other features requires python.\nWould you like to install it?") == 0:
                if not cmdExists("pip") and cmdExists("python"):
                    os.system("python")
                else:
            ...
            print("ERR\nTYP: {0}\nARG: {1}".format(args[1], tuple(args[2:])))
        elif args[0] == "crash":
            log("User Requested Crash", "FATAL")
            if len(args) > 1:
                if len(args) > 2:
                    log(" ".join(args[2:]), "CRASH_LOG")
            ...
            if os.path.exists(f"{os.getenv("APPDATA")}/TerminalPlus/{ENVI}/DEV"):
                if args[1] == "commit" or args[1] == "push":
                    ...
                    temp1 = f.selPrompt(["Major", "Minor", "Micro", "None"], ["+.0.0", "x.+.0", "x.x.+", "x.x.x"], f"{c.Fore.BLUE}Current Version: {VER}{c.Style.RESET_ALL}\nWhat type of commit is this?\n(x: digit stays the same, +: digit increments, 0: digit resets to 0, max digit 99)\nMajor: System Reworks, New Set of Commands\nMinor: Update System, New Command\nMicro: Edit Command, New Subcommand")
                    ...
                    print(f"{c.Fore.BLUE}New Version: {c.Fore.CYAN}{".".join(temp2)}{c.Style.RESET_ALL} (Change to {["Major", "Minor", "Micro", "None"][temp1]})")
                    
                    with open(f"{homePath}/code/data.py", "w") as temp1:
                        temp1.write(f"env = \"{ENVI}\"\nver = \"{".".join(temp2)}\"\npy_ver = {str(PREF_VER)}")
                        temp1.close()
